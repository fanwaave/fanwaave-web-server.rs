...
#
# fanwaave-web-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/fanwaave/fanwaave-web-server:dev --load .
#
...
#     -t ghcr.io/fanwaave/fanwaave-web-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/fanwaave/fanwaave-web-server:dev .
#
#   `just docker-build` / `just docker-run` / `just docker-push` wrap all three.
#
# DEPENDENCIES — 2 crate(s) come from git remotes (Cargo.toml:
# fanwaave-lib-core, flags2env) and 0 from zed-pkg (.zpkg.toml: none).
...
#     ghcr.io/fanwaave/fanwaave-web-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    cargo build --release --bin fanwaave-web-server; \
    strip target/release/fanwaave-web-server; \
    cp target/release/fanwaave-web-server /usr/local/bin/fanwaave-web-server
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/fanwaave-web-server /usr/local/bin/fanwaave-web-server
ENV FANWAAVE_WEB_BIND=0.0.0.0:8081
EXPOSE 8081
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/fanwaave-web-server"]
...
COPY --from=builder /usr/local/bin/fanwaave-web-server /usr/local/bin/fanwaave-web-server
...
CMD ["/usr/local/bin/fanwaave-web-server"]
