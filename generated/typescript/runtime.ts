...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    ...
    json: (() => { const raw = lookup("FANWAAVE_JSON"); return raw === undefined ? undefined : parseBool(raw, false); })(),
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const api_base = pick(["FANWAAVE_API_BASE"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (api_base !== undefined) out["FANWAAVE_API_BASE"] = api_base;
  const config = pick(["FANWAAVE_CONFIG"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (config !== undefined) out["FANWAAVE_CONFIG"] = config;
  const fanwaave_api_http_base = pick(["FANWAAVE_API_HTTP_BASE"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (fanwaave_api_http_base !== undefined) out["FANWAAVE_API_HTTP_BASE"] = fanwaave_api_http_base;
  const fanwaave_database_url = pick(["FANWAAVE_DATABASE_URL"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (fanwaave_database_url !== undefined) out["FANWAAVE_DATABASE_URL"] = fanwaave_database_url;
  const fanwaave_web_bind = pick(["FANWAAVE_WEB_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (fanwaave_web_bind !== undefined) out["FANWAAVE_WEB_BIND"] = fanwaave_web_bind;
  const json = pick(["FANWAAVE_JSON"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (json !== undefined) out["FANWAAVE_JSON"] = json;
  return out;
}
