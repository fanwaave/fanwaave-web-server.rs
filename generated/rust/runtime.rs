...
pub struct CliEnvValues {
    ...
    pub fanwaave_api_http_base: Option<String>,
    pub fanwaave_database_url: Option<String>,
    pub fanwaave_web_bind: Option<String>,
...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        api_base: lookup("FANWAAVE_API_BASE").filter(|value| !value.is_empty()),
        config: lookup("FANWAAVE_CONFIG").filter(|value| !value.is_empty()),
        fanwaave_api_http_base: lookup("FANWAAVE_API_HTTP_BASE").filter(|value| !value.is_empty()),
        fanwaave_database_url: lookup("FANWAAVE_DATABASE_URL").filter(|value| !value.is_empty()),
        fanwaave_web_bind: lookup("FANWAAVE_WEB_BIND").filter(|value| !value.is_empty()),
        json: lookup("FANWAAVE_JSON").map(|raw| parse_bool(Some(raw), false)),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let api_base = pick(&["FANWAAVE_API_BASE"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = api_base {
        out.insert("FANWAAVE_API_BASE".to_string(), value);
    }
    let config = pick(&["FANWAAVE_CONFIG"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = config {
        out.insert("FANWAAVE_CONFIG".to_string(), value);
    }
    let fanwaave_api_http_base = pick(&["FANWAAVE_API_HTTP_BASE"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = fanwaave_api_http_base {
        out.insert("FANWAAVE_API_HTTP_BASE".to_string(), value);
    }
    let fanwaave_database_url = pick(&["FANWAAVE_DATABASE_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = fanwaave_database_url {
        out.insert("FANWAAVE_DATABASE_URL".to_string(), value);
    }
    let fanwaave_web_bind = pick(&["FANWAAVE_WEB_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = fanwaave_web_bind {
        out.insert("FANWAAVE_WEB_BIND".to_string(), value);
    }
    let json = pick(&["FANWAAVE_JSON"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = json {
        out.insert("FANWAAVE_JSON".to_string(), value);
    }
    Ok(out)
}
