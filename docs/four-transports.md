# The four avenues between `fanwaave-web-server` and `fanwaave-api-server`

`fanwaave` follows the fleet-wide contract in
...
|---|------|------|-------------------|
| 1 | `direct_read` | `fanwaave-web-server` → Postgres, via `fanwaave-lib-core` | a read on the page's critical path |
| 2 | `http` | `fanwaave-web-server` → `fanwaave-api-server`, stateless | the default, and anything that writes |
| 3 | `tcp` | `fanwaave-web-server` ⇄ `fanwaave-api-server`, held open | chatty internal calls where the handshake dominates |
| 4 | `jet_stream` | `fanwaave-web-server` → NATS → `fanwaave-api-server` | work the caller does not need to watch finish |

All four carry the same envelope and land on the same handler, so a
`fanwaave-e2e` suite can drive one operation four ways and assert the answers
...
|---|---|
| Slug | `fanwaave` |
| Environment prefix | `FANWAAVE_` |
| Request subject | `fanwaave.api.operation.v1` |
| Result subject | `fanwaave.api.operation-result.v1` |
| Durable consumer | `fanwaave-api-server-v1` |
...
|----------|--------|--------------|
| `FANWAAVE_READONLY_DATABASE_URL` | 1 | no direct reads |
| `FANWAAVE_API_URL` | 2 | no stateless HTTP |
| `FANWAAVE_API_MTLS_ADDR` | 3 | no stateful TCP |
| `FANWAAVE_API_TLS_SERVER_NAME` | 3 | plaintext to a TLS-terminating mesh |
| `FANWAAVE_WEB_CLIENT_CERT_FILE` | 3 | ” |
| `FANWAAVE_WEB_CLIENT_KEY_FILE` | 3 | ” |
| `FANWAAVE_API_CA_FILE` | 3 | ” |
| `FANWAAVE_NATS_URL` | 4 | no asynchronous avenue |

A missing variable disables its avenue. A variable that is present and unsafe
fails startup: a plaintext `FANWAAVE_API_URL` off loopback, a
`FANWAAVE_NATS_URL` that is not `tls://`, or mutual TLS with two of its
four files set.

In-cluster, `FANWAAVE_NATS_URL` points at the messaging service defined in
...
**In `fanwaave-web-server`** — implement `DirectReader` over `fanwaave-lib-core`:
...
impl DirectReader<Operation, Projection> for LibCoreReader {
        ...
        -> Result<Projection, TransportError>
    {
        ...
        // not an access check.
        // Then build the query with fanwaave-lib-core's query builders.
    }
...
**In `fanwaave-api-server`** — implement `OperationHandler` once, and hand the same
...
**Avenue 1 is read-only twice over.** `fanwaave-lib-core` gates its write surface
behind a `read-write` cargo feature and migrations behind `migrate`. Depend on
it from `fanwaave-web-server` as:

```toml
fanwaave-lib-core = { git = "…", default-features = false, features = ["read-only"] }
```

so a mutating call is a *compile error*, not a code-review miss. Separately,
`FANWAAVE_READONLY_DATABASE_URL` must name a Postgres role without
`INSERT`, `UPDATE`, `DELETE` or DDL. A mistake has to defeat both. Migrations
belong to `fanwaave-lib-core` and `declarative-migrations`, never to a web server.
